...
int kvs_write(size_t num_pairs, char keys[][MAX_STRING_SIZE],
              char values[][MAX_STRING_SIZE], client_t *clients[]) {
  if (kvs_table == NULL) {
    fprintf(stderr, "KVS state must be initialized\n");
    return 1;
  }
  char notif_message[2 + 2 * MAX_STRING_SIZE] = {0};
  ...
  for (size_t i = 0; i < num_pairs; i++) {
    if (write_pair(kvs_table, keys[i], values[i]) != 0) {
      ...
      for (int j = 0; j < MAX_SESSION_COUNT; j++){
        if (clients[j] != NULL){
          for (int k = 0; k < clients[j]->count_keys; k++){
            if (strcmp(clients[j]->subscribed_keys[k], keys[i]) == 0){
              snprintf(notif_message, MAX_STRING_SIZE, "%s", keys[i]);
              snprintf(notif_message + 1 + MAX_STRING_SIZE, MAX_STRING_SIZE, "%s", values[i]);

              if(write_all(clients[j]->notif_pipe_fd, notif_message, sizeof(notif_message)) == -1){
                perror("[ERR]: Failed to send notification to client");
              }
            }
          }
        }
...
int kvs_delete(size_t num_pairs, char keys[][MAX_STRING_SIZE], int fd, client_t *clients[]) {
  if (kvs_table == NULL) {
    fprintf(stderr, "KVS state must be initialized\n");
    return 1;
  }
  ...
  for (size_t i = 0; i < num_pairs; i++) {
    if (delete_pair(kvs_table, keys[i]) != 0) {
      ...
      snprintf(str, MAX_STRING_SIZE, "(%s,KVSMISSING)\n", keys[i]);
      write_str(fd, str);
          ...
          if(clients[j] != NULL){
            for(int k = 0; k < clients[j]->count_keys; k++){
              if(strcmp(clients[j]->subscribed_keys[k], keys[i]) == 0){
                key_place = k;
                snprintf(notif_message, MAX_STRING_SIZE, "%s", keys[i]);
                snprintf(notif_message + 1 + MAX_STRING_SIZE, sizeof("DELETED"), "DELETED");

                if(write_all(clients[j]->notif_pipe_fd, notif_message, sizeof(notif_message)) == -1){
                  perror("[ERR]: Failed to send notification to client");
                }

                for (int m = key_place; m < clients[j]->count_keys; m++){
                  strncpy(clients[j]->subscribed_keys[m], clients[j]->subscribed_keys[m + 1], sizeof(clients[j]->subscribed_keys[m]));
                }  
                clients[j]->count_keys --;
              }
            }
          }
        }
...
//if key exists return 1, key doesnt exist return 0

int verify_keys(char keys[MAX_STRING_SIZE]) {
  if (kvs_table == NULL) {
    fprintf(stderr, "KVS state must be initialized\n");
    return 0;
  }
  ...
  char *result = read_pair(kvs_table, keys);
  if (result == NULL){
    free(result);
    pthread_rwlock_unlock(&kvs_table->tablelock);
    return 0;
