...
/*void remove_subs(int client_id){
  for (int i = 0; i < MAX_CLIENTS; i++){
    strcmp(client_subs[i].client_id, client_id){
      printf("removing subs for client %s", client_id)

      memset(client_subscriptions[i].subscribed_keys, 0,sizeoff(client_subscriptions[i].subscribed_keys));
      client_subscriptions[i].num_keys = 0;
      break;
    }
  }
}*/
...
int client_handler(client_t *client){
  ...
  char response_message[2] = {0}; // 1 byte para OP_CODE e 1 byte para result
  response_message[0] = (char) OP_CODE_CONNECT; // Definir o OP_CODE
  response_message[1] = result;
  ...
  while(1){
    char buffer[42]; // buffer para ler o OP_CODE
    char op_code;
    //char keys[MAX_WRITE_SIZE][1] = {0};
    ssize_t bytes_read = read(req_pipe, &op_code, sizeof(buffer));
    if (bytes_read == 0){
      break;
    }
    OP_CODE = buffer[0];
    ...
    switch (OP_CODE){
      ...
      case '2':{
        int client_id;
        handle_disconnect(req_pipe, client_id);
      }

      case '3':{
        char key[MAX_STRING_SIZE]={0};
        if(read(req_pipe, key, sizeoff(key)) <= 0){
          perror("[ERR]: Failed to read key for subscription");
          break;
        }
        if (handle_subscribe(key,resp_pipe) != 0){
      ...
      case '4':{
        ...
        if (handle_unsubscribe(key, resp_pipe) !=0){
          fprintf(stderr, "[ERR]: Unsubscription handling failed for key '%s'\n", key);
        }
      }
    }
    if (bytes_read == -1){
      perror("[ERR]: Failed to read OP_CODE");
      return EXIT_FAILURE;
    }
  }
}
...
static void dispatch_threads(DIR *dir, const char *server_pipe_path) {
  ...
  int server_pipe_fd = open(server_pipe_path, O_RDONLY, O_WRONLY);
  if ((server_pipe_fd) == -1){
    perror("[ERR]: server_pipe open failed");
    //Fechar os pipes
    return EXIT_FAILURE;
